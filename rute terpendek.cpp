...
void dijkstra(const vector<vector<Node>> &graph, int startNode, vector<int> &distances) {
  ...
  while (!pq.empty()) {
    ...
    for (const Node &edge : graph[currNode]) {
...
int main() {
  ...
  vector<vector<Node>> graph(numNodes + 1);
